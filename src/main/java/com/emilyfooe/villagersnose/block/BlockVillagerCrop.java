...
public class BlockVillagerCrop extends CropsBlock {
    ...
    public BlockVillagerCrop() {
        super(Block.Properties.of(Material.PLANT).noCollission().randomTicks().sound(SoundType.CROP).harvestLevel(0));
    }
    ...
    public void destroy(IWorld worldIn, BlockPos pos, BlockState state) {
        if (isMaxAge(state)) {
            ...
            worldIn.addFreshEntity(new ItemEntity((World) worldIn, pos.getX(), pos.getY(), pos.getZ(), stack));
        }
    }
    ...
    private static void spawnVillager(World world, BlockPos pos) {
        ...
        if (villagerEntity != null) {
            villagerEntity.setPos(pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5);
            //addVillagerSpawnEffects(world, villagerEntity.posX, villagerEntity.posY, villagerEntity.posZ);

            world.addFreshEntity(villagerEntity);
        }
    }
...
//        world.playSound(posX, posY, posZ, SoundEvents.VILLAGER_CELEBRATE, SoundCategory.NEUTRAL, 1.0F, 1.0F, false);
//    }
}
